...
#构造20%缺失的原始数据
# ======================
# ⭐ 30% time-step mask（只针对 TP）
# ======================
def apply_tp_time_mask(X_val, X_val_original, target_idx, ratio=0.3, seed=42):
    ...
    X_val_original_masked = X_val_original.copy()

    # 选择时间步 mask
    mask_t = np.ones(T, dtype=bool)
    mask_idx = np.random.choice(T, int(T * ratio), replace=False)
    mask_t[mask_idx] = False

    # ======================
    # 只作用在 TP
    # ======================
    X_val_masked[:, target_idx, :][:, ~mask_t] = np.nan
    X_val_original_masked[:, target_idx, :][:, ~mask_t] = np.nan

    return X_val_masked, X_val_original_masked, mask_t


X_val, X_val_original, tp_time_mask = apply_tp_time_mask(
    X_val,
    X_val_original,
    target_idx,
    ratio=0.2,
...
val_features, _, val_t_idx, val_ob_mask, _ = generate_dataset(
    X_val,
    X_val_original,
    num_timesteps_input,
...
# ⭐ 全局缺失 mask（唯一正确的 global mask）
missing_mask_global = (true_full == 0) | np.isnan(true_full)
...
# ======================
def aggregate_last(pred, t_idx, original_shape):
...
# ======================
def build_final_series(true_full, pred_full):
    ...
    return final, missing_mask

def plot_result(node_idx, true_full, pred_full, missing_mask,time_index):

    plt.figure(figsize=(12,5))

    t = time_index  # ⭐ 使用 test 对齐时间轴

    # t = np.arange(true_full.shape[1])

    red_line = true_full[node_idx].copy()

    # ❗只在缺失位置用预测值填补
    red_line[missing_mask[node_idx]] = pred_full[node_idx][missing_mask[node_idx]]
    ...
    # ❗把缺失位置变成 NaN（强制断线）
    true_plot[missing_mask[node_idx]] = np.nan
    ...
    png_path = os.path.join('predictions_interp/node_series', f'node_{node_idx}.png')
    plt.savefig(png_path, dpi=300, bbox_inches='tight')

    svg_path=os.path.join('predictions_interp/node_series', f'node_{node_idx}.svg')
...
final_series, missing_mask_global = build_final_series(
    true_full,
    val_pred_full
)
